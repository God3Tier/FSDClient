...
public partial class Home : Control
{
	...
	private void get_packs()
	{
		var Packs = GetNode<HFlowContainer>("Packs");
		
		// update to actual ASYNC function call
		String[] PackData = {"Legendary", "Common", "Common"};
		
		for (int i = 0; i< Packs.GetChildCount(); i++)
		{
			var PackSlot = Packs.GetChild(i) as ColorRect;
			
			var PackImage = PackSlot.GetNode<TextureRect>("PackImage");
			// see what kind of pack
			switch(PackData[i]){
				case "Common":
					PackImage.Texture = GD.Load<Texture2D>("res://assets/cards/CommonPack.png");
					break;
				case "Rare":
					PackImage.Texture = GD.Load<Texture2D>("res://assets/cards/RarePack.png");
					break;
				case "Epic":
					PackImage.Texture = GD.Load<Texture2D>("res://assets/cards/EpicPack.png");
					break;
				case "Legendary":
					PackImage.Texture = GD.Load<Texture2D>("res://assets/cards/LegendaryPack.png");
					break;
				default:
					PackImage.Texture = GD.Load<Texture2D>("res://assets/cards/pack.png");
					break;
			}
		}
	}
